...
beam hook claude-code < payload.json # forward a Claude Code hook payload (stdin)
```
...
- `GET /agents` — existence checks on common agent config/artifact locations; no contents are read.
