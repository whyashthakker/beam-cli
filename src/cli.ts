...
program.command("hook")
  .description("Forward a Claude Code PreToolUse/PostToolUse/UserPromptSubmit hook payload from stdin (observation only, never blocks the agent)")
  .argument("[source-agent]", "label for the reporting agent", "claude-code")
  .action(captureHook);
